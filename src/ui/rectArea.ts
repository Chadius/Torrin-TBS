...
export const RectAreaService = {
    ...
    isInside(rectArea: RectArea, coordinateX: number, coordinateY: number): boolean {
        return (
...
const setRectTop = (rectArea: RectArea, params: RectTop): void => {
    const baseRectangleIsValid = (params as BaseRectangle).baseRectangle && isValidValue((params as BaseRectangle).baseRectangle.top);
    const paramIsValid = {
...
const setRectLeft = (rectArea: RectArea, params: RectLeft): void => {
    const baseRectangleIsValid = (params as BaseRectangle).baseRectangle && isValidValue((params as BaseRectangle).baseRectangle.left);
    const paramIsValid = {
        baseRectangle: baseRectangleIsValid,
...
const setRectHeight = (rectArea: RectArea, params: RectHeight): void => {
    const paramIsValid = {
        baseRectangle: (params as BaseRectangle).baseRectangle && isValidValue((params as BaseRectangle).baseRectangle.height),
        positionHeight: (params as PositionHeight) && isValidValue((params as PositionHeight).height),
        topBottom: (params as RectHeightTopBottom) && isValidValue(rectArea.top) && isValidValue((params as RectHeightTopBottom).bottom),
        ...
        percentTopBottom: (params as RectHeightTopPercentBottom) && isValidValue(rectArea.top) && isValidValue((params as RectHeightTopPercentBottom).percentBottom) && isValidValue((params as RectHeightTopPercentBottom).screenHeight),
    };
    ...
    if (paramIsValid.positionHeight) {
        ...
        rectArea.height = paramValue.percentHeight.screenHeight * paramValue.percentHeight.percentHeight / 100;
    } else if (paramIsValid.percentTopBottom) {
        rectArea.height = ((paramValue.percentTopBottom.screenHeight * paramValue.percentTopBottom.percentBottom) / 100) - rectArea.top;
    }
    ...
    if (
        ...
        if (paramIsValid.baseRectangle) {
            ...
            rectArea.height = baseRectangle.height - marginValues1[0] - marginValues1[2];
        } else {
...
const setRectWidth = (rectArea: RectArea, params: RectWidth): void => {
    const paramIsValid = {
        baseRectangle: (params as BaseRectangle).baseRectangle && isValidValue((params as BaseRectangle).baseRectangle.width),
        positionWidth: (params as PositionWidth) && isValidValue((params as PositionWidth).width),
        leftRight: (params as RectWidthLeftRight) && isValidValue(rectArea.left) && isValidValue((params as RectWidthLeftRight).right),
        ...
        percentLeftRight: (params as RectWidthLeftPercentRight) && isValidValue(rectArea.top) && isValidValue((params as RectWidthLeftPercentRight).percentRight) && isValidValue((params as RectWidthLeftPercentRight).screenWidth),
        columnLeftRight: (params as RectWidthLeftColumnEnd) && isValidValue((params as RectWidthLeftColumnEnd).screenWidth) && isValidValue((params as RectWidthLeftColumnEnd).endColumn) && isValidValue(rectArea.left),
    };
    ...
    if (paramIsValid.positionWidth) {
        ...
        rectArea.width = paramValue.percentWidth.screenWidth * paramValue.percentWidth.percentWidth / 100;
    } else if (paramIsValid.percentLeftRight) {
        rectArea.width = ((paramValue.percentLeftRight.screenWidth * paramValue.percentLeftRight.percentRight) / 100) - rectArea.left;
    } else if (paramIsValid.columnLeftRight) {
        rectArea.width = (paramValue.columnLeftRight.screenWidth * (paramValue.columnLeftRight.endColumn + 1) / 12) - rectArea.left;
    }
    ...
    if (
        ...
        const marginValues1 = marginValues(marginsAll);
        if (paramIsValid.baseRectangle) {
            rectArea.width = paramValue.baseRectangle.width - marginValues1[1] - marginValues1[3];
        } else {
