import {HexDirection, HexGridHelper, MoveCoordinatesInOneDirection, moveOneTileInDirection} from "./hexGridDirection";
...
describe('Hex Grid Helper', () => {
    ...
    describe('Generate offsets of a certain radius', () => {
        it('generates a ring of 0 around the origin', () => {
            const radius0: HexCoordinate[] = HexGridHelper.GetCoordinatesForRingAroundOrigin(0);
            expect(radius0).toHaveLength(1);
            expect(radius0).toContainEqual({q: 0, r: 0});
        });
        it('generates a ring of 1 around the origin', () => {
            const radius1: HexCoordinate[] = HexGridHelper.GetCoordinatesForRingAroundOrigin(1);
        ...
        it('generates a ring of 2 around the origin', () => {
            const radius2: HexCoordinate[] = HexGridHelper.GetCoordinatesForRingAroundOrigin(2);
        ...
        it('generates a ring around a given coordinate', () => {
            const radius2: HexCoordinate[] = HexGridHelper.GetCoordinatesForRingAroundCoordinate({q: 3, r: 5}, 2);
            expect(radius2).toHaveLength(12);
