...
describe('Reachable Squaddies Results', () => {
    it('can add squaddies by distance', () => {
        ...
        expect(soldierDistanceResults.closestCoordinatesByDistance.getCoordinatesByDistance(0)).toContainEqual({
            q: 0,
            r: 0
        });
        expect(soldierDistanceResults.closestCoordinatesByDistance.getCoordinatesByDistance(1)).toHaveLength(2);
        expect(soldierDistanceResults.closestCoordinatesByDistance.getCoordinatesByDistance(1)).toContainEqual({
            q: 0,
            r: 1
        });
        expect(soldierDistanceResults.closestCoordinatesByDistance.getCoordinatesByDistance(1)).toContainEqual({
        ...
        expect(soldierDistanceResults.closestCoordinatesByDistance.getCoordinatesByDistance(2)).toContainEqual({
            q: 0,
            r: 2
        });
        expect(soldierDistanceResults.closestCoordinatesByDistance.getCoordinatesByDistance(2)).toContainEqual({
