...
describe('pathfinder and squaddies', () => {
    ...
    it('knows which squaddies it was adjacent to', () => {
        ...
        expect(enemyNearby.closestCoordinatesByDistance[1]).toContainEqual({q: 0, r: 1});
        expect(enemyNearby.closestCoordinatesByDistance[1]).toContainEqual({q: 1, r: 1});
        expect(enemyNearby.closestCoordinatesByDistance[1]).toContainEqual({q: 1, r: 2});
        expect(enemyNearby.closestCoordinatesByDistance[1]).not.toContainEqual({q: 0, r: 2}); // Cannot pass through enemy
        expect(enemyNearby.closestCoordinatesByDistance[1]).not.toContainEqual({q: 0, r: 3}); // Cannot stop on ally

        const allyFlankingNearby = reachableSquaddies.getCoordinatesCloseToSquaddieByDistance("ally_flanking");
        expect(allyFlankingNearby.closestCoordinatesByDistance[0]).toContainEqual({q: 0, r: 3}); // 0 distance can pass through ally
        expect(allyFlankingNearby.closestCoordinatesByDistance[1]).toContainEqual({q: 1, r: 2});
        expect(allyFlankingNearby.closestCoordinatesByDistance[1]).toContainEqual({q: 1, r: 3});
        expect(allyFlankingNearby.closestCoordinatesByDistance[1]).not.toContainEqual({q: 0, r: 2}); // Cannot stop on enemy
        expect(allyFlankingNearby.closestCoordinatesByDistance[1]).not.toContainEqual({q: 0, r: 4}); // Out of movement

        const allyAtTheEdge = reachableSquaddies.getCoordinatesCloseToSquaddieByDistance("ally_at_the_edge");
        expect(allyAtTheEdge.closestCoordinatesByDistance[1]).toContainEqual({q: 1, r: 3});
        expect(allyAtTheEdge.closestCoordinatesByDistance[0]).toBeUndefined(); // Out of movement, cannot reach ally's location
