...
const isTimeExpired = (state: DialoguePlayerState): boolean => {
    // TODO check to see if dialogue has an animation duration
...
const createUIObjects = (state: DialoguePlayerState, context: TextSubstitutionContext) => {
    ...
    state.answerButtons = answerButtonPositions.map((position, index) => new DialogueAnswerButton({
        ...
        position: position,
        screenDimensions: [ScreenDimensions.SCREEN_WIDTH, ScreenDimensions.SCREEN_HEIGHT], // TODO get rid of this
    }));
...
export class TODODeleteMeDialogueBoxPlayer implements CutsceneAction {
    id: string;
