...
import {MockedP5GraphicsBuffer, MockedP5GraphicsRenderer} from "../utils/test/mocks";
...
describe("user clicks on the map to move", () => {
    ...
    beforeEach(() => {
        ...
        selectorAndHUDClickOnSquaddie(selector, battleSquaddieSelectedHUD, playerBattleSquaddie.battleSquaddieId, gameEngineState);
    });
    ...
    describe('Invalid locations', () => {
        ...
        it('When User clicks in range on another squaddie', () => {
            ...
            MissionMapService.addSquaddie(missionMap, anotherPlayer.squaddieTemplateId, anotherPlayer.battleSquaddieId, {
                q: 0,
                r: 2
            });
    ...
    describe('When user clicks on map to make a legal move', () => {
        ...
        it('Squaddie Selector will create a new route', () => {
            expect(gameEngineState.battleOrchestratorState.battleState.squaddieMovePath).not.toBeUndefined();
            const squaddieMovePath = gameEngineState.battleOrchestratorState.battleState.squaddieMovePath;
        ...
        it('Squaddie Selector will create a Processed Action describing the movement, for the given squaddie, giving it to ActionsThisRound', () => {
            ...
            expect(actionsThisRound.processedActions[0].decidedAction).toEqual(DecidedActionService.new({
                actionPointCost: 3,
            ...
            expect(actionsThisRound.processedActions[0].processedActionEffects).toHaveLength(1);
            expect(actionsThisRound.processedActions[0].processedActionEffects[0]).toEqual(ProcessedActionMovementEffectService.new({
        ...
        it('Squaddie spends the action points moving and moves to the new location', () => {
            ...
            const {mapLocation} = missionMap.getSquaddieByBattleId(playerBattleSquaddie.battleSquaddieId);
    ...
    describe('Squaddie Mover knows to move over', () => {
        ...
        let graphicsContext: GraphicsRenderer;
        let moveSquaddieAlongPathSpy: SpyInstance;

        beforeEach(() => {
            selectorClicksOnMapLocation(selector, gameEngineState, 0, 3);
            mover = new BattleSquaddieMover();
            graphicsContext = new MockedP5GraphicsRenderer();
        ...
        it('mover tries to move the squaddie icon', () => {
            ...
            expect(moveSquaddieAlongPathSpy.mock.calls[0][0].battleSquaddie.battleSquaddieId).toEqual(playerBattleSquaddie.battleSquaddieId);
        });
    });
});
...
const selectorAndHUDClickOnSquaddie = (selector: BattlePlayerSquaddieSelector, battleSquaddieSelectedHUD: BattleSquaddieSelectedHUD, battleSquaddieId: string, gameEngineState: GameEngineState) => {
...
const selectorClicksOnMapLocation = (selector: BattlePlayerSquaddieSelector, gameEngineState: GameEngineState, q: number, r: number) => {
    let [mouseX, mouseY] = convertMapCoordinatesToScreenCoordinates(q, r, ...gameEngineState.battleOrchestratorState.battleState.camera.getCoordinates());
    selector.mouseEventHappened(gameEngineState, {
        eventType: OrchestratorComponentMouseEventType.CLICKED,
        mouseX,
        mouseY,
