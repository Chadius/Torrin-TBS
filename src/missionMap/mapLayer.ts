...
export const MapLayerHelper = {
        ...
        for (let q = 0; q < newMapLayer.numberOfRows; q++) {
            ...
            for (let r = 0; r < newMapLayer.widthOfWidestRow; r++) {
                if (
                    ...
                    newMapLayer.valueByLocation[q][r] = initialValue as number | boolean | undefined;
                } else {
                    const initialValueFunction = initialValue as MapCoordinateValueGenerator;
                    newMapLayer.valueByLocation[q][r] = initialValueFunction(q, r);
                }
            }
        }
    ...
    setValueOfLocation: ({mapLayer, q, r, value}: {
        mapLayer: MapLayer,
        q: number;
        r: number;
        ...
        if (outOfBounds({mapLayer, q, r})) {
            throw new Error(`(${q}, ${r}) is out of bounds, must be within (${mapLayer.numberOfRows}, ${mapLayer.widthOfWidestRow})`);
        }

        mapLayer.valueByLocation[q][r] = value;
    },
    outOfBounds: ({mapLayer, q, r}: { mapLayer: MapLayer, q: number; r: number }): boolean => {
        return outOfBounds({mapLayer, q, r});
    }
}

const outOfBounds = ({mapLayer, q, r}: { mapLayer: MapLayer, q: number; r: number; }): boolean => {
    return (
        q < 0
