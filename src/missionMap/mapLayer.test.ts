...
describe('MapLayer', () => {
    it('can generate a layer based on a given map and initial value', () => {
        ...
        [0, 1, 2, 3, 4].forEach(r => {
            [0, 1].forEach(q => {
                expect(mapLayerFilledWithFalse.valueByLocation[q][r]).toBe(false);
            });
        })
    });
    ...
    it('can generate a layer based on a given map and value filling function', () => {
        ...
        [0, 1, 2, 3, 4].forEach(r => {
            [0, 1].forEach(q => {
                expect(mapLayerFilledWithSumOfCoordinates.valueByLocation[q][r]).toBe(q + r);
            });
        })
    });
    ...
    it('can generate a layer based on a given map and use undefined if given', () => {
        ...
        [0, 1, 2, 3, 4].forEach(r => {
            [0, 1].forEach(q => {
                expect(mapLayerFilledWithUndefined.valueByLocation[q][r]).toBe(undefined);
            });
        })
    });
    ...
    it('can set the value of an individual tile on the layer', () => {
        ...
        MapLayerHelper.setValueOfLocation({mapLayer: mapLayerFilledWithFalse, q: 1, r: 2, value: true});
    ...
    it('throws an error if setting a location that is out of bounds', () => {
        ...
        const shouldThrowError = () => {
            MapLayerHelper.setValueOfLocation({mapLayer: mapLayerFilledWithFalse, q: 9001, r: -3, value: true});
        }
    ...
    it('knows when a location is out of bounds', () => {
        ...
        [0, 1, 2, 3, 4].forEach(r => {
            [0, 1].forEach(q => {
                expect(MapLayerHelper.outOfBounds({mapLayer: mapLayerFilledWithFalse, q, r})).toBe(false);
            });
        });
        expect(MapLayerHelper.outOfBounds({mapLayer: mapLayerFilledWithFalse, q: -1, r: 0})).toBe(true);
        expect(MapLayerHelper.outOfBounds({
            mapLayer: mapLayerFilledWithFalse,
            q: -1,
            r: mapLayerFilledWithFalse.widthOfWidestRow
        })).toBe(true);
        expect(MapLayerHelper.outOfBounds({mapLayer: mapLayerFilledWithFalse, q: 0, r: -1})).toBe(true);
        expect(MapLayerHelper.outOfBounds({
            mapLayer: mapLayerFilledWithFalse,
