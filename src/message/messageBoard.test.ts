...
describe('MessageBoard', () => {
    it('can register listeners and retrieve them', () => {
        ...
        const messageBoardListener = new TestMessageListener("testMessageListener");
        messageBoard.addListener(messageBoardListener, MessageBoardMessageType.BASE);
        expect(messageBoard.getListenerById(messageBoardListener.messageBoardListenerId)).toBe(messageBoardListener);
        expect(messageBoard.getListenersByMessageType(MessageBoardMessageType.BASE)).toEqual([messageBoardListener]);
    });

    it('returns nothing if there are no messages with the message id or type', () => {
        const messageBoard = new MessageBoard();
        expect(messageBoard.getListenersByMessageType(MessageBoardMessageType.BASE)).toHaveLength(0);
    ...
    it('calls listener function if the id is used', () => {
        ...
        const messageReceivedSpy = jest.spyOn(messageBoardListener, "receiveMessage");
    ...
    it('removes listeners by id', () => {
        ...
        messageBoard.removeListenerById(messageBoardListener.messageBoardListenerId);

        expect(messageBoard.getListenerById(messageBoardListener.messageBoardListenerId)).toBeUndefined();
    });
});
