...
describe('ActionEffectSquaddieTemplate', () => {
    it('can be constructed using data object', () => {
        const newAction: ActionEffectSquaddieTemplate = ActionEffectSquaddieTemplateService.new({
        ...
        expect(newAction.traits).toEqual(TraitStatusStorageService.newUsingTraitValues({[Trait.ATTACK]: true}));
    });
    ...
    describe('sanitize', () => {
        it('can be sanitized to fill in missing fields', () => {
            const actionWithMissingFields = ActionEffectSquaddieTemplateService.new({
