...
export const DecidedActionService = {
    ...
    areDecisionsRequired: (decidedAction: DecidedAction): boolean => {
        ...
        const mostRecentDecidedActionEffect: DecidedActionEffect = decidedAction.actionEffects[decidedAction.actionEffects.length - 1];
        switch (mostRecentDecidedActionEffect.type) {
            case ActionEffectType.SQUADDIE:
                return DecidedActionSquaddieEffectService.areDecisionsRequired(mostRecentDecidedActionEffect);
            case ActionEffectType.MOVEMENT:
                return DecidedActionMovementEffectService.areDecisionsRequired(mostRecentDecidedActionEffect);
...
const sanitize = (action: DecidedAction): DecidedAction => {
    if (!isValidValue(action.battleSquaddieId)) {
        throw new Error("DecidedAction cannot sanitize, missing battleSquaddieId");
    }
