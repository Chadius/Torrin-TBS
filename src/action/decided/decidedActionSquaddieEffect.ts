...
export const DecidedActionSquaddieEffectService = {
    ...
    areDecisionsRequired: (
        actionEffect: DecidedActionSquaddieEffect
    ): boolean => {
        return !isValidValue(actionEffect.target)
    },
    getMultipleAttackPenalty: (
        decidedActionSquaddieEffect: DecidedActionSquaddieEffect
    ): number => {
        return ActionEffectSquaddieTemplateService.getMultipleAttackPenalty(
            decidedActionSquaddieEffect.template
        )
    },
}
