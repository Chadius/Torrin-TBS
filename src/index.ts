...
export const sketch = (p: p5) => {
    p.setup = () => {
        ...
        canvas = p.createCanvas(ScreenDimensions.SCREEN_WIDTH, ScreenDimensions.SCREEN_HEIGHT);
        frameBuffer = p.createGraphics(ScreenDimensions.SCREEN_WIDTH, ScreenDimensions.SCREEN_HEIGHT)
        frameBuffer.colorMode("hsb", 360, 100, 100, 255)
        p.frameRate(60)
        canvas.elt.addEventListener("contextmenu", (e: { preventDefault: () => any; }) => e.preventDefault())
        gameEngine = new GameEngine({graphicsBuffer: frameBuffer, startupMode: StartupMode});
        gameEngine.setup({
