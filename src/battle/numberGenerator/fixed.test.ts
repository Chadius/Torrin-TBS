...
describe('fixed number generator', () => {
    it('will always produce the given number', () => {
        const generatorAlwaysReturns5: NumberGeneratorStrategy = new FixedNumberGenerator({result: 5});
        expect(generatorAlwaysReturns5.next()).toBe(5);

        const generatorAlwaysReturns255: NumberGeneratorStrategy = new FixedNumberGenerator({result: 255});
    ...
    it('will throw an error if it is created with an out of range number', () => {
        ...
        expect(() => {
            shouldThrowErrorTooHigh()
        }).toThrow("Fixed Number Generator must set number between 1 and 360, got: 9001");
        ...
        expect(() => {
            shouldThrowErrorTooLow()
        }).toThrow("Fixed Number Generator must set number between 1 and 360, got: 0");
