...
export const ActionResultText = {
    ...
    getActingSquaddieRollTotalIfNeeded: (result: SquaddieSquaddieResults): string[] => {
        return getActingSquaddieRollTotalIfNeeded(result);
    },
};

const getAttackPenaltyDescriptions = (actingSquaddieModifiers: { [modifier in ATTACK_MODIFIER]?: number }): string[] => {
...
const getActingSquaddieRollTotalIfNeeded = (result: SquaddieSquaddieResults): string[] => {
    ...
    return [
        ` Total ${totalAttackRoll + totalModifier}`
    ];
};
