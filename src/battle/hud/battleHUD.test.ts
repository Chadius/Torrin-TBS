...
import {BattleHUDListener, BattleHUDService, PopupWindowType} from "./battleHUD";
...
describe('Battle HUD', () => {
    ...
    describe('draw', () => {
        let mockGraphicsContext: MockedP5GraphicsRenderer
        beforeEach(() => {
            mockGraphicsContext = new MockedP5GraphicsRenderer();
        })
    ...
    describe('Popup Windows', () => {
        it('can set a popup window', () => {
            ...
            expect(battleHUD.popupWindows[PopupWindowType.DIFFERENT_SQUADDIE_TURN]).toEqual(differentSquaddiePopup)
        })
        it('will create a popup window if it gets a message that the player selected a different squaddie', () => {
            ...
            MissionMapService.addSquaddie(missionMap, squaddieTemplate.squaddieId.templateId, battleSquaddie.battleSquaddieId, {
                q: 0,
                r: 0
            })
            ...
            expect(gameEngineState.battleOrchestratorState.battleHUD.popupWindows[PopupWindowType.DIFFERENT_SQUADDIE_TURN]).not.toBeUndefined()

            const popup = gameEngineState.battleOrchestratorState.battleHUD.popupWindows[PopupWindowType.DIFFERENT_SQUADDIE_TURN]
            expect(popup.label.textBox.text.includes(`${squaddieTemplate.squaddieId.name}\n is not done yet`)).toBeTruthy()
        })
    })
})
