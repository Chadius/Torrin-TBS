...
describe('File Access HUD', () => {
    ...
    describe('enable and disable buttons based on gameEngineState', () => {
        ...
        it('should disable the save and load buttons during the player phase if a squaddie is taking a turn', () => {
            takingATurnSpy = jest.spyOn(OrchestratorUtilities, "isSquaddieCurrentlyTakingATurn").mockReturnValue(true);
            const gameEngineState: GameEngineState = createGameEngineStateWithBattlePhase(BattlePhase.PLAYER);
            FileAccessHUDService.updateBasedOnGameEngineState(fileAccessHUD, gameEngineState);
    ...
    describe('clicking on Save Game', () => {
        ...
        describe('save is completed successfully', () => {
            beforeEach(() => {
                SaveSaveStateService.savingAttemptIsComplete(fileState.saveSaveState);
            ...
            it('enables the button after save completes and the message expires', () => {
                FileAccessHUDService.updateStatusMessage(fileAccessHUD, fileState);
                expectNoMessageAfterDisplayDuration(dateSpy, fileState, fileAccessHUD);
                FileAccessHUDService.updateBasedOnGameEngineState(fileAccessHUD, createGameEngineStateWithBattlePhase(BattlePhase.PLAYER));
                expect(fileAccessHUD.saveButton.getStatus()).toEqual(ButtonStatus.READY);
                expect(fileAccessHUD.saveButtonCachedStatus).toBeUndefined();
            });
        });
        describe('save has an error during saving', () => {
            beforeEach(() => {
                SaveSaveStateService.foundErrorDuringSaving(fileState.saveSaveState);
                FileAccessHUDService.updateButtonStatus(fileAccessHUD);
                dateSpy = jest.spyOn(Date, 'now').mockReturnValue(0);
            });
    ...
    describe('clicking on Load Game', () => {
        ...
        describe('load is completed successfully', () => {
            ...
            it('enables the button after load completes and the message expires', () => {
                ...
                expect(fileAccessHUD.loadButtonCachedStatus).toBeUndefined();
            });
        });
...
const expectNoMessageAfterDisplayDuration = (dateSpy: jest.SpyInstance<any, any>, fileState: FileState, fileAccessHUD: FileAccessHUD) => {
    dateSpy = jest.spyOn(Date, 'now').mockReturnValue(FileAccessHUDDesign.MESSAGE_DISPLAY_DURATION + 1);
    const noMessage: string = FileAccessHUDService.updateStatusMessage(fileAccessHUD, fileState);
