...
describe('BattleCamera', () => {
    it('can be constrained so it cannot scroll too high up', () => {
        const camera: BattleCamera = new BattleCamera(0, -SCREEN_HEIGHT);
        const numberOfRowsThatCanFitOnScreen = Math.trunc(SCREEN_HEIGHT / HEX_TILE_WIDTH);
        camera.setYVelocity(100);
        camera.setMapDimensionBoundaries(1, numberOfRowsThatCanFitOnScreen + 5);

        camera.constrainCamera();

        expect(camera.getCoordinates()[1]).toBeGreaterThanOrEqual(0 - SCREEN_HEIGHT / 10 + SCREEN_HEIGHT / 2);
        expect(camera.getVelocity()[1]).toBe(0);
    });

    it('can be constrained so it cannot scroll too far down', () => {
        const camera: BattleCamera = new BattleCamera(0, 2 * SCREEN_HEIGHT);
        const numberOfRowsThatCanFitOnScreen = Math.trunc(SCREEN_HEIGHT / HEX_TILE_WIDTH);
        camera.setYVelocity(-100);
        camera.setMapDimensionBoundaries(1, numberOfRowsThatCanFitOnScreen + 5);
    ...
    it('can be constrained so it cannot scroll too far to the left', () => {
        ...
        const camera: BattleCamera = new BattleCamera(-SCREEN_WIDTH * 2, worldLocationOfBoundary[1]);
        camera.setXVelocity(100);
        camera.setMapDimensionBoundaries(2, 5);

        camera.constrainCamera();

        expect(camera.getCoordinates()[0]).toBeGreaterThanOrEqual(worldLocationOfBoundary[0] - SCREEN_WIDTH / 10);
    ...
    it('can be constrained so it cannot scroll too far to the right', () => {
        ...
        expect(camera.getCoordinates()[0]).toBeLessThanOrEqual(worldLocationOfBoundary[0] + SCREEN_WIDTH / 10);
        expect(camera.getVelocity()[0]).toBe(0);
    });
});