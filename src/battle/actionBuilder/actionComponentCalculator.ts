...
export const ActionComponentCalculator = {
    getNextOrchestratorComponentMode: (actionBuilderState: PlayerBattleActionBuilderState): BattleOrchestratorMode => {
        ...
        const actionIsEndTurn = PlayerBattleActionBuilderStateService.getAction(actionBuilderState).endTurn === true
        if (actionIsEndTurn) {
            return BattleOrchestratorMode.SQUADDIE_USES_ACTION_ON_MAP;
        }

        const actionIsMovement = PlayerBattleActionBuilderStateService.getAction(actionBuilderState).movement === true
        const targetIsConfirmed = PlayerBattleActionBuilderStateService.isTargetConfirmed(actionBuilderState) === true
    ...
    getNextModeBasedOnActionsThisRound: (actionsThisRound: ActionsThisRound): BattleOrchestratorMode => {
        const processedActionEffect = ActionsThisRoundService.getProcessedActionEffectToShow(actionsThisRound);
    ...
    getPendingActionDecisions: (actionBuilderState: PlayerBattleActionBuilderState): ActionDecisionType[] => {
        if (!isValidValue(actionBuilderState)) {
            return [ActionDecisionType.ACTOR_SELECTION, ActionDecisionType.ACTION_SELECTION]
        }

        let actorAndActionMissing: ActionDecisionType[] = [];
        if (!PlayerBattleActionBuilderStateService.isActorSet(actionBuilderState)) {
            actorAndActionMissing.push(ActionDecisionType.ACTOR_SELECTION);
        }
        if (!PlayerBattleActionBuilderStateService.isActionSet(actionBuilderState)) {
