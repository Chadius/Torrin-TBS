...
describe('PlayerHUDController', () => {
    ...
    describe('Change mode based on selected inputs', () => {
        ...
        beforeEach(() => {
            ...
            gameEngineState.battleOrchestratorState.battleState.teams.push(playerTeam);
            ...
            gameEngineState.battleOrchestratorState.battleState.teams.push(enemyTeam);
        ...
        it('recommends computer selector mode when no players can take a turn and there is no pending animation', () => {
            ...
            const recommendedChanges = controller.recommendStateChanges(gameEngineState);
            expect(recommendedChanges.nextMode).toEqual(BattleOrchestratorMode.COMPUTER_SQUADDIE_SELECTOR);
        });
        it('recommends player squaddie selector when the actor is not set', () => {
            const recommendedChanges = controller.recommendStateChanges(gameEngineState);
            expect(recommendedChanges.nextMode).toEqual(BattleOrchestratorMode.PLAYER_SQUADDIE_SELECTOR);
        });
        ...
        it('recommends player squaddie target when a action that needs a target is set', () => {
            ...
            const recommendedChanges = controller.recommendStateChanges(gameEngineState);
            expect(recommendedChanges.nextMode).toEqual(BattleOrchestratorMode.PLAYER_SQUADDIE_TARGET);
        });
