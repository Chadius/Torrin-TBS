...
export class BattleSquaddieMover implements BattleOrchestratorComponent {
    ...
    private updateWhileAnimationIsInProgress(state: GameEngineState, graphicsContext: GraphicsRenderer) {
        const {
            battleSquaddie,
    ...
    private updateWhenAnimationCompletes(gameEngineState: GameEngineState, graphicsContext: GraphicsRenderer) {
        const {
            squaddieTemplate,
            battleSquaddie,
...
const updateIconAndMapBasedOnWhetherSquaddieCanAct = (state: GameEngineState, battleSquaddie: BattleSquaddie, squaddieTemplate: SquaddieTemplate, graphicsContext: GraphicsRenderer) => {
    const mapIcon = state.repository.imageUIByBattleSquaddieId[battleSquaddie.battleSquaddieId];
    if (!mapIcon) {
        return;
    }
